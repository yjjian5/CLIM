...
def eval_seq(opt, dataloader, data_type, result_filename, save_dir=None, show_image=True, frame_rate=30):
    ...
    # TODO: keep pre_image, keep pre_hm
    pre_blob = None
    pre_hm = None
    ...
    for i, (path, img, img0) in enumerate(dataloader):
        ...
        # todo: use previous blob and heatmap
        if count != 0:
            online_targets, pre_hm_ = tracker.update(blob, img0, pre_blob, pre_hm)   # pre_hm?
        else:
            online_targets, pre_hm_ = tracker.update(blob, img0)

        # copy current blob and heatmap
        pre_blob = blob.clone().detach()
        pre_hm = pre_hm_.clone().detach()
...
def main(opt, data_root='', det_root=None, seqs=(''), exp_name='demo',
