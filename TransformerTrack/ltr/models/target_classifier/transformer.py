...
class Transformer(nn.Module):
    ...
    def forward(self, train_feat, test_feat, train_label):
        num_img_train = train_feat.shape[0]     # train_feat_shape: 3, 5, 512, 22, 22
...
class TransformerEncoderLayer(nn.Module):
    ...
    def instance_norm(self, src, input_shape):
        ...
        src = src.reshape(-1, dim, h, w)    # 15, 512, 22, 22
        src = self.norm(src)    # 15, 512, 22, 22
        # reshape back
        src = src.reshape(num_imgs, batch, dim, -1).permute(0,3,1,2)    # 3, 484, 5, 512
        src = src.reshape(-1, batch, dim)   # 1452, 5 ,512
        return src

    def forward(self, src, input_shape, pos: Optional[Tensor] = None):
        # query = key = value = src
        query = src     # 1452, 5, 512 same as follows
        key = src
        value = src
    
        # self-attention
        src2 = self.self_attn(query=query, key=key, value=value)       # 1452, 5, 512
        src = src + src2    # 1452, 5, 512
        src = self.instance_norm(src, input_shape)
        return src      # 1452, 5, 512
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src, pos: Optional[Tensor] = None):
        ...
        src_shape = src.shape
        num_imgs, batch, dim, h, w = src.shape      # 3, 5, 512, 22, 22

        src = src.reshape(num_imgs, batch, dim, -1).permute(0,3,1,2)        # 3, 484, 5, 512
        src = src.reshape(-1, batch, dim)       # 1452, 5, 512

        if pos is not None:     # None
        ...
        return output, output_feat      # 1452, 5, 512; 15, 512, 22, 22
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, input_shape, pos: Optional[Tensor] = None, query_pos: Optional[Tensor] = None):
        # self-attention
        query = tgt     # 484, 5, 512 same as follows
        key = tgt
        value = tgt
        
        tgt2 = self.self_attn(query=query, key=key, value=value)
        tgt = tgt + tgt2
        tgt = self.instance_norm(tgt, input_shape)      # 484,5,512
    
        mask = self.cross_attn(query=tgt, key=memory, value=pos)    # 484,5,512
        tgt2 = tgt * mask
        tgt2 = self.instance_norm(tgt2, input_shape)    # 484,5,512
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory, pos: Optional[Tensor] = None, query_pos: Optional[Tensor] = None):
        ...
        tgt_shape = tgt.shape
        num_imgs, batch, dim, h, w = tgt.shape      # 1, 5, 512, 22, 22

        if pos is not None:     # 3, 5, 22, 22
            num_pos, batch, h, w = pos.shape
            pos = pos.view(num_pos, batch, 1, -1).permute(0,3,1,2) # 3,484,5,1
            pos = pos.reshape(-1, batch, 1)     # 1452,5,1
            pos = pos.repeat(1, 1, dim)     # 1452, 5, 512

        tgt = tgt.view(num_imgs, batch, dim, -1).permute(0,3,1,2)   # 1,484,5,512
        tgt = tgt.reshape(-1, batch, dim)       # 484, 5, 512
        ...
        # [L,B,D] -> [B,D,L]
        output_feat = output.reshape(num_imgs, h, w, batch, dim).permute(0,3,4,1,2)     # 1,5,512,22,22
        output_feat = output_feat.reshape(-1, dim, h, w)
        # output = self.post2(self.activation(self.post1(output)))
        return output, output_feat  # 484,5,512; 5,512,22,22
